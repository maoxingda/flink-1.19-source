...
class FlinkCost(
  ...
  override def multiplyBy(factor: Double): RelOptCost = {
    ...
    new FlinkCost(rowCount * factor, cpu * factor, io * factor, network * factor, memory * factor)
  }
  ...
  override def divideBy(cost: RelOptCost): Double = {
    ...
    var n: Double = 0.0
    if (
      (this.rowCount != 0) && !this.rowCount.isInfinite &&
    ...
    if (
      (this.network != 0) && !this.network.isInfinite &&
    ...
    if (
      (this.memory != 0) && !this.memory.isInfinite &&
