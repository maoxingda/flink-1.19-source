...
/**
 * A [[RelNodeBlock]] is a sub-tree in the [[RelNode]] DAG, and represents common sub-graph in
 * [[CommonSubGraphBasedOptimizer]]. All [[RelNode]]s in each block have only one [[LegacySink]]
 * output.
 *
 * The algorithm works as follows:
 *   1. If there is only one tree, the whole tree is in one block. (the next steps is needless.) 2.
 *      reuse common sub-plan in different RelNode tree, generate a RelNode DAG, 3. traverse each
 *      tree from root to leaf, and mark the sink RelNode of each RelNode 4. traverse each tree from
 ...
 *      and FlinkLogicalCorrelate will be combined into a BatchExecCorrelate or a
 *      StreamExecCorrelate.
 *
 * For example: (Table API)
 *
 * {{{- val sourceTable = tEnv.scan("test_table").select('a, 'b, 'c) val leftTable =
 * sourceTable.filter('a > 0).select('a as 'a1, 'b as 'b1) val rightTable =
 * sourceTable.filter('c.isNotNull).select('b as 'b2, 'c as 'c2) val joinTable =
 * leftTable.join(rightTable, 'a1 === 'b2) joinTable.where('a1 >= 70).select('a1,
 * 'b1).writeToSink(sink1) joinTable.where('a1 < 70 ).select('a1, 'c2).writeToSink(sink2) }}}
 *
 * the RelNode DAG is:
 *
 * {{{- Sink(sink1) Sink(sink2) \| | Project(a1,b1) Project(a1,c2) \| | Filter(a1>=70) Filter(a1<70)
 * \ / Join(a1=b2) / \ Project(a1,b1) Project(b2,c2) \| | Filter(a>0) Filter(c is not null) \ /
 * Project(a,b,c) \| TableScan }}}
 *
 * This [[RelNode]] DAG will be decomposed into three [[RelNodeBlock]]s, the break-point is the
 * [[RelNode]](`Join(a1=b2)`) which data outputs to multiple [[LegacySink]]s. <p>Notes: Although
 * `Project(a,b,c)` has two parents (outputs), they eventually merged at `Join(a1=b2)`. So
 * `Project(a,b,c)` is not a break-point. <p>the first [[RelNodeBlock]] includes TableScan,
 * Project(a,b,c), Filter(a>0), Filter(c is not null), Project(a1,b1), Project(b2,c2) and
 * Join(a1=b2) <p>the second one includes Filter(a1>=70), Project(a1,b1) and Sink(sink1) <p>the
 * third one includes Filter(a1<70), Project(a1,c2) and Sink(sink2) <p>And the first
 * [[RelNodeBlock]] is the child of another two.
 *
 * The [[RelNodeBlock]] plan is: {{{- RelNodeBlock2 RelNodeBlock3 \ / RelNodeBlock1}}}
 *
 * The optimizing order is from child block to parent. The optimized result (RelNode) will be
 * wrapped as an IntermediateRelTable first, and then be converted to a new TableScan which is the
...
class RelNodeBlockPlanBuilder private (tableConfig: ReadableConfig) {
  ...
  /**
   * TableFunctionScan/Snapshot/Window Aggregate cannot be optimized individually, so
   * TableFunctionScan/Snapshot/Window Aggregate is not a valid break-point even though it has
   * multiple parents.
   */
  ...
  private def buildBlockOutputNodes(
    ...
    if (wrapper.allParentNodesVisited) {
      ...
      node.getInputs.foreach {
        input => buildBlockOutputNodes(Some(node), input, newBlockOutputNode, unvisitedNodeQueue)
      }
...
object RelNodeBlockPlanBuilder {
   ...
   * to RelNode trees. Second, reuse same sub-plan in different trees. Third, decompose the RelNode
   * dag to [[RelNodeBlock]] trees.
   *
   * @param sinkNodes
   *   SinkNodes belongs to a LogicalNode plan.
   * @return
   *   Sink-RelNodeBlocks, each Sink-RelNodeBlock is a tree.
   */
  /**
